...
function determinCity(){
    ...
    const APIKEY = '122f759f3073bc3b285d10fb134175c4';
...
function displayWeatherForecast(weatherData) {
    ...
    for (let i = 0; i < weatherData.length; i += 2) {
        ...
        for (let j = i; j < i + 2 && j < weatherData.length; j++) {
            ...
            dayContainer.innerHTML = `
                <p>${dayData.date}</p>
                <p>Max Temperature: ${dayData.maxTemperature}°C</p>
                <p>Min Temperature: ${dayData.minTemperature}°C</p>
                <p>Wind Direction: ${dayData.windDirection}°</p>
                <p>Sunrise: ${dayData.sunrise}</p>
                <p>Sunset: ${dayData.sunset}</p>
            `;

            rowContainer.appendChild(dayContainer);
        }

        // Append the row container to the main weather container
        weatherContainer.appendChild(rowContainer);
    }
}


displayWeatherForecast(sampleWeatherData);
